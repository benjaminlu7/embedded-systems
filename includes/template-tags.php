...
/*
================================================================================================
Embedded Systems - template-tags.php
================================================================================================
...
================================================================================================
*/
function embedded_systems_entry_posted_on_setup() {
    $author_avatar_size = apply_filters('embedded_systems_author_avatar_size', 80);
    ...
    esc_attr(sprintf(__('View all posts by %s', 'embedded-systems'), get_the_author())), 
    get_the_author()
    ));

    if ( !is_page() && !post_password_required() && (comments_open() || get_comments_number())) {
        echo '<span class="entry-comments"><b>';
            comments_popup_link( sprintf( __( 'Add a Comment', 'embedded-systems')));
        echo '</b></span>';
    }
}
...
================================================================================================
*/
function embedded_systems_entry_time_stamp_setup() {
	...
	return sprintf(
		__('<span class="screen-reader-text">Posted on</span> %s', 'embedded-systems'),
...
================================================================================================
*/
function embedded_systems_entry_taxonomies_setup() {
    $cat_list = get_the_category_list(__(' | ', 'embedded-systems'));
    $tag_list = get_the_tag_list('', __(' | ', 'embedded-systems'));

    if ($cat_list) {
        printf('<div class="cat-link"> %1$s <span class="cat-list"l><b><i>%2$s</i></b></span></div>',
        __('<i class="fa fa-folder-open-o"></i> Posted In', 'embedded-systems'),  
        $cat_list
        );
    }
    ...
    if ($tag_list) {
        ...
        __('<i class="fa fa-tags"></i> Tagged', 'embedded-systems'),  
        $tag_list 
        );
    }
}